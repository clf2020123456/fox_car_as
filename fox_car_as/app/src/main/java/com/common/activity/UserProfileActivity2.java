...
public class UserProfileActivity2 extends Activity {
	...
	@ViewInject(R.id.editIdenno)
	private EditText editIdenno;
	...
	private MyApp myApp;
	private String zan,zan_,money,jsonString,car_status,image_url;

	@Override  
	protected void onCreate(Bundle savedInstanceState) {
		// TODO Auto-generated method stub
		super.onCreate(savedInstanceState);
		myApp = (MyApp) UserProfileActivity2.this.getApplication();
		setContentView(R.layout.activity_user_update2);
			...
			public void onFocusChange(View v, boolean hasFocus) {
				if (hasFocus) {
					...
					if (TextUtils.isEmpty(username.getText().toString().trim())) {
						Toast.makeText(UserProfileActivity2.this, "用户名不能为空", 0)
								.show();
					} else if (username.getText().toString().trim().length() < 4
							|| username.getText().toString().trim().length() > 20) {
						Toast.makeText(UserProfileActivity2.this,
...
//						checkUser(username.getText().toString().trim());
	...
	public void onclick(View view) {
		...
		case R.id.register:
			...
			String carstatus = editStatus.getText().toString();
			String phone = editPhone.getText().toString().trim();
			String name = editName.getText().toString().trim();
			String address = editAddress.getText().toString().trim();
			String idenno = editIdenno.getText().toString().trim();
		/*	if(!isMobile(phone)){
				Util.showToast(this, "手机号码格式输入不正确");
				return;
				
			}*/
			SendData(user_name, password, qq, phone,name,address,carstatus,idenno);
			break;
		...
		public void handleMessage(android.os.Message msg) {
			if (msg.what == 0x125) {
				...
				try {
					...
					if(obj.length()> 0){
						...
						editIdenno.setText(idenno);
						editStatus.setText(car_status);
			...
			if (msg.what == 0x127) {
				...
				if(jsonString.equals("1")){
					Toast.makeText(UserProfileActivity2.this, "恭喜，信息保存成功", Toast.LENGTH_SHORT).show();;
	            	Intent intent = new Intent(UserProfileActivity2.this,TabHostActivity.class);
	            	UserProfileActivity2.this.startActivity(intent);
				}else{
					Toast.makeText(UserProfileActivity2.this, "抱歉，操作失败", Toast.LENGTH_SHORT).show();;
				}
			}
			if (msg.what == 0x128) {
			}
	...
	public void SendData(final String username , final String password ,final String qq,final String phone,final String name,final String address,final String car_status,final String idenno){
		...
		new Thread() {
			public void run() {
				try {
					...
					query+="user.car_status="+car_status+"&";
			...
			public void updateUI(Serializable runData) {
				...
				if (runData != null) {
					...
					if (map.get("type").equals("success")) {
						...
						Util.Toast(UserProfileActivity2.this, "用户名已经存在。");
						username.setText("");
					}
				} else {
					Util.Toast(UserProfileActivity2.this, "网络连接错误");
				}
			}
