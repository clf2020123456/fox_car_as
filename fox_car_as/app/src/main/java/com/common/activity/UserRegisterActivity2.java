			...
			public void onFocusChange(View v, boolean hasFocus) {
				if (hasFocus) {
					...
					if (TextUtils.isEmpty(username.getText().toString().trim())) {
						Toast.makeText(UserRegisterActivity2.this, "用户名不能为空", 0)
								.show();
					} else if (username.getText().toString().trim().length() < 4
							|| username.getText().toString().trim().length() > 20) {
						Toast.makeText(UserRegisterActivity2.this,
...
//						checkUser(username.getText().toString().trim());
	...
	public void onclick(View view) {
		...
		case R.id.register:
			...
			SendData(user_name, password, qq, phone,name,address,carstatus,idenno,sex);
			break;
	...
	private Handler handler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			if (msg.what == 0x126) {
				//
				Toast.makeText(UserRegisterActivity2.this, "恭喜，信息保存成功", Toast.LENGTH_SHORT).show();;
            	Intent intent = new Intent(UserRegisterActivity2.this,Login.class);
            	UserRegisterActivity2.this.startActivity(intent);
			}
			if (msg.what == 0x127) {
				Toast.makeText(UserRegisterActivity2.this, "抱歉，操作失败", Toast.LENGTH_SHORT).show();;
			}
			if (msg.what == 0x128) {
				Toast.makeText(UserRegisterActivity2.this, "抱歉，该用户名已被占用", Toast.LENGTH_SHORT).show();;
			}
			if (msg.what == 0x129) {
				Toast.makeText(UserRegisterActivity2.this, "抱歉，该手机号已被注册", Toast.LENGTH_SHORT).show();;
			}
		};
	};
	public void SendData(final String username , final String password , final String qqnum, final String phone, final String name, final String address,
						 final String car_status, final String idenno,final String sex){
			...
			public void updateUI(Serializable runData) {
				...
				if (runData != null) {
					...
					if (map.get("type").equals("success")) {
						...
						Util.Toast(UserRegisterActivity2.this, "用户名已经存在。");
						username.setText("");
					}
				} else {
					Util.Toast(UserRegisterActivity2.this, "网络连接错误");
				}
			}
