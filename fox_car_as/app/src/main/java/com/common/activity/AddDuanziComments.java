...
/*		StrictMode.setThreadPolicy(new StrictMode.ThreadPolicy.Builder()
	...
	public void onclick(View view) {
		...
		case R.id.register:
			...
			SensitivewordFilter filter = new SensitivewordFilter(AddDuanziComments.this);
			Set<String> set = filter.getSensitiveWord(gamecontext.getText().toString().trim(), 1);

			if(set.size()>0){
				Toast.makeText(AddDuanziComments.this, "检测到您发布的信息里面有"+set.size()+"个网络敏感词,请重新输入", Toast.LENGTH_SHORT).show();;
				return;
			}
